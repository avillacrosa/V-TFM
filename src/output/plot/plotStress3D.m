function plotStress3D(Geo, Mat, Set, Result)
	figure
	hold on
	plot(Result.time, reshape(max(Result.s(:,1,:),[],1), size(Result.time)), 'o');
	plot(Result.time, reshape(max(Result.s(:,2,:),[],1), size(Result.time)), 'o');
	plot(Result.time, reshape(max(Result.s(:,3,:),[],1), size(Result.time)), 'o');
...
% 	end
	...
	figure
	hold on
	plot(Result.time, reshape(max(Result.s(:,4,:),[],1), size(Result.time)), 'o');
	plot(Result.time, reshape(max(Result.s(:,5,:),[],1), size(Result.time)), 'o');
	plot(Result.time, reshape(max(Result.s(:,6,:),[],1), size(Result.time)), 'o');
