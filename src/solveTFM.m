function Result = solveTFM(Geo, Mat, Set, Result)
	...
	se_t	 = zeros(Geo.vect_dim, Geo.n_elem, Geo.n_nodes_elem, Set.time_incr);

	t = 0;
	for k = 1:Set.time_incr-Set.dk
        [T, ~, ~, ~] = internalF(k, X + u_t, s_t, se_t, Q_t, Geo, Mat, Set);

    	R = T - F_t(:,k+Set.dk);
    	[u_t, s_t, se_t, Q_t] = newton(k, u_t, s_t, se_t, Q_t, R, F_t, Geo, Mat, Set);
        if mod(k, Set.save_freq) == 0
			c = k/Set.save_freq + 1;
            Result = saveOutData(t, c, k, u_t, s_t, se_t, F_t, T, M, Geo, Mat, Set, Result);
